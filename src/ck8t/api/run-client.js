/**
 * Client for executing a ck8t workflow against the convengine backend.
 *
  ...
  if (!res.ok) {
    if (res.status === 404) {
      const rich = new Error(
        ...
        'Make sure the convengine-demo app is running and the ck8t-server package is deployed.'
      )
      rich.url = url
      rich.resolvedUrl = resolveUrl(url)
      rich.method = 'POST'
