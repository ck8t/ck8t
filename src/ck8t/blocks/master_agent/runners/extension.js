function topoSortSteps(steps) {
  const layers = [], resolved = new Set()
  let remaining = [...steps], guard = steps.length + 2
  while (remaining.length > 0 && guard-- > 0) {
    const ready = remaining.filter(s => s.depends_on.every(d => resolved.has(d)))
    if (ready.length === 0) { layers.push(remaining); break }
    layers.push(ready); ready.forEach(s => resolved.add(s.id))
...
export default [
  {
    type: 'master_agent',
    async run({ values, input, callAgent, allNodes, subBlockValues, nodeId, nodeTitle }) {
      ...
      if (!rawModel) throw new Error(`No model configured for Master Agent "${nodeTitle}". Open Settings → LLM Provider Configuration.`)
      ...
      const planRes = await callAgent({ agent: { id: (nodeId || 'master') + '_plan', model: rawModel, temperature: 0.3, systemPrompt: planSystemPrompt, userPrompt: question }, input: question })
      let steps = []
      try {
      ...
      for (const layer of layers) {
        await Promise.all(layer.map(async step => {
          ...
          const res = await callAgent({ agent: { id: (nodeId || 'master') + '_' + step.id, model: rawModel, temperature: 0.3, systemPrompt: `You are a ${step.capability} specialist. Answer concisely.`, userPrompt: slaveInput }, input: slaveInput })
      ...
      const synthRes = await callAgent({ agent: { id: (nodeId || 'master') + '_synthesis', model: rawModel, temperature: 0.2, systemPrompt: synthSystem, userPrompt: synthUserPrompt }, input: synthUserPrompt })
      let finalAnswer = synthRes.output, confidence = 0.8
      try {
