// Extension-side runner for the Chain of Thought block.
export async function run({ node, values, input, callAgent }) {
  ...
  if (!rawModel) throw new Error(`No model configured for Chain of Thought "${node?.data?.title || node?.id}". Open Settings → LLM Provider Configuration.`)
  ...
  let parsed = {}
  try {
    const res = await callAgent({ agent: { id: node?.id || 'cot', model: rawModel, temperature: 0.2, systemPrompt, userPrompt }, input: inputStr })
