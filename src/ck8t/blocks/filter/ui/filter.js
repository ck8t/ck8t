...
export const FilterBlock = defineCk8tBlock({
  ...
  tools: { access: [] },
  inputs: {
    items: { type: 'any', description: 'Array of items to filter' },
    conditions: { type: 'string', description: 'Filter function or expression' },
    mode: { type: 'string', description: 'Keep or remove matches' },
  },
  outputs: {
    kept: { type: 'array', description: 'Items that matched the filter' },
    rejected: { type: 'array', description: 'Items that did not match' },
